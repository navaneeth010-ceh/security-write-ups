...
The login endpoint leaks information through **response differentiation**:
- Invalid username → "Invalid username"
- Valid username + wrong password → "Incorrect password"

By comparing responses, an attacker can determine which usernames exist without ever 
guessing a password. This is **information disclosure** disguised as security messaging.
...
- Same HTTP status code (200) regardless of failure
- Same response length (pad if needed)
- Same response time (use constant-time password comparison)
- **Rate limiting** on login attempts (throttle, CAPTCHA, account lockout)